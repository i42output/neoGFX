    ...
    void standard_shape_shader::generate_code(const i_shader_program& aProgram, shader_language aLanguage, i_string& aOutput) const
    {
        ...
        if (aLanguage == shader_language::Glsl)
        {
            static const string code
            {
                ...
                "}\n"
                "\n"
                "vec2 fn_cubic_bezier(float t)\n"
                "{\n"
                "    return pow(1.0 - t, 3.0) * uShapeP0 + 3.0 * pow(1.0 - t, 2.0) * t * uShapeP1 + 3.0 * (1.0 - t) * pow(t, 2.0) * uShapeP2 + pow(t, 3.0) * uShapeP3;\n"
                "}\n"
                // todo: this implementation is rubbish: we need a kosher (axis independent) algorithm to find distance of point on a plane to the curve.
                "void draw_cubic_bezier(inout vec4 color, inout vec4 function)\n"
                "{\n"
                "    float dx = uShapeP3.x - uShapeP0.x;\n"
                "    float dy = uShapeP3.y - uShapeP0.y;\n"
                "    if (abs(dx) >= abs(dy))\n"
                "    {\n"
                "        float t = (Coord.x - uShapeP0.x) / dx;\n"
                "        vec2 xy = fn_cubic_bezier(t);\n"
                "        float d = distance(Coord.xy, vec2(Coord.x, xy.y));\n"
                "        color = vec4(color.xyz, color.a * (1.0 - smoothstep(uShapeWidth / 2.0 - 0.5, uShapeWidth / 2.0 + 0.5, d)));\n"
                "    }\n"
                "    else\n"
                "    {\n"
                "        float t = (Coord.y - uShapeP0.y) / dy;\n"
                "        vec2 xy = fn_cubic_bezier(t);\n"
                "        float d = distance(Coord.xy, vec2(xy.x, Coord.y));\n"
                "        color = vec4(color.xyz, color.a * (1.0 - smoothstep(uShapeWidth / 2.0 - 0.5, uShapeWidth / 2.0 + 0.5, d)));\n"
                "    }\n"
                "}\n"
                "\n"
