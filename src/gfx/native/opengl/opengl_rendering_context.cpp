...
namespace neogfx
{
...

        {
            ...
            for (auto op = aDrawRectOps.cbegin(); op != aDrawRectOps.cend(); ++op)
            {
                ...
                if (usp == std::nullopt)
                {
                    ...
                    maybeVertexArrays.emplace(as_vertex_provider<>(*this), *this, static_cast<std::size_t>(2u * 3u * (aDrawRectOps.cend() - aDrawRectOps.cbegin())));
                }
...

        {
            ...
            for (auto op = aDrawCheckerboardOps.cbegin(); op != aDrawCheckerboardOps.cend(); ++op)
            {
                ...
                for (std::uint32_t pass = 0u; pass <= 1u; ++pass)
                {
                    ...
                    maybeVertexArrays.emplace(as_vertex_provider<>(*this), *this, static_cast<std::size_t>(2u * 3u));
    ...
    void opengl_rendering_context::draw_glyphs(const draw_glyph* aBegin, const draw_glyph* aEnd)
    {
        ...
        auto shape_quad = [&](font const& glyphFont, glyph_char const& glyphChar, bool outline = false) -> quadf_2d
            {
                ...
                if (!outline)
                {
                    ...
                    shapeQuad = glyphChar.outlineShape.value_or(glyphChar.shape);
        ...
        std::optional<scoped_blending_mode> blendingMode;
            ...
            draw_glyphs_stage::Adornments })
        {
            ...
            case draw_glyphs_stage::Paper:
                for (auto const& drawOp : std::ranges::subrange(aBegin, aEnd))
                {
                    ...
                    auto& textEffect = drawOp.appearance->effect();
                    auto& textEffect2 = drawOp.appearance->effect2();

                    auto consider = [&](optional_text_effect const& aEffect)
                        {
                            if (!aEffect)
                                return false;
                            switch (aEffect->type())
                    ...
                    if (consider(textEffect) || consider(textEffect2))
                    {
                        if (!filterRegion)
                ...
                if (filterRegion)
                {
                    for (auto effect : { 0, 1 })
                    {
                        ...
                        for (auto const& drawOp : std::ranges::subrange(aBegin, aEnd))
                        {
                            ...
                            auto const& textEffect = (effect == 0 ? drawOp.appearance->effect() : drawOp.appearance->effect2());
                            if (!textEffect)
                                continue;

                            if (textEffect->type() != text_effect_type::Glow &&
                                textEffect->type() != text_effect_type::Shadow &&
                            ...
                            if (is_emoji(glyphChar) && textEffect->ignore_emoji())
                                continue;

                            bool const renderEffects = !drawOp.appearance->only_calculate_effect() && textEffect;
                            if (!renderEffects)
                                continue;
            ...
            case draw_glyphs_stage::EmojiOutline:
                if (filterRegion)
                {
                    for (auto effect : { 0, 1 })
                    {
                        std::optional<scoped_filter<dilate_filter>> filter;
                        scalar opacity = 1.0;
                        ...
                        for (auto const& drawOp : std::ranges::subrange(aBegin, aEnd))
                        {
                            ...
                            if (!is_emoji(glyphChar))
                                continue;

                            auto const& textEffect = (effect == 0 ? drawOp.appearance->effect() : drawOp.appearance->effect2());

                            if (!textEffect || textEffect->ignore_emoji())
                                continue;

                            if (textEffect->type() != text_effect_type::Outline)
                                continue;

                            if (drawOp.appearance->only_calculate_effect())
                                continue;

                            auto const radius = glyphFont.info().outline().radius;
