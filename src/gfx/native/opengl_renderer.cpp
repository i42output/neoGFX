				...
				shaders
				{
					...
					std::make_pair(
						std::string(
							...
							"	ivec2 dtpos = ivec2((vGlyphTexCoord - glyphTextureOffset) / glyphTextureExtents * glyphDestinationTextureExtents);\n"
							"	if (guiCoordinates)\n"
							"		dtpos.y = int(glyphDestinationTextureExtents.y) - 1 - dtpos.y;\n"
							"   vec4 rgbDestination = texelFetch(glyphDestinationTexture, dtpos, 0);\n"
