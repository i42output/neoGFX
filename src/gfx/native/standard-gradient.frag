...
vec4 filter_gradient_color(in float n)
{
    n = max(min(n, 1.0), 0.0) * float(uFilterGradientColorCount - 1);
    vec4 firstColor = texelFetch(uGradientColors, ivec2(floor(n), uFilterGradientColorRow));
    vec4 secondColor = texelFetch(uGradientColors, ivec2(ceil(n), uFilterGradientColorRow));
    return mix(firstColor, secondColor, n - floor(n));
}
                
float filter_ellipse_radius(vec2 ab, vec2 center, vec2 pt)
{
    vec2 d = pt - center;
    float angle = 0;
    vec2 ratio = vec2(1.0, 1.0);
    if (ab.x >= ab.y)
        ...
        ratio.x = ab.y / ab.x;
    angle = atan(d.y * ratio.y, d.x * ratio.x);
    float x = pow(abs(cos(angle)), 2.0 / uFilterGradientExponents.x) * sign(cos(angle)) * ab.x;
    float y = pow(abs(sin(angle)), 2.0 / uFilterGradientExponents.y) * sign(sin(angle)) * ab.y;
    return sqrt(x * x + y * y);
}
                
float filter_gradient_pos_at(vec2 viewPos, vec4 boundingBox)
{
    vec2 s = boundingBox.zw - boundingBox.xy;
    vec2 pos = viewPos - boundingBox.xy;
    if (uFilterGradientTile)
    {
        if (uFilterGradientDirection != 1) /* vertical */
        {
            float adjust = 0;
            if (uFilterGradientTileParams.z == 1)
                adjust = float(int(boundingBox.y) % uFilterGradientTileParams.y);
            float frac = 1 / float(uFilterGradientTileParams.y);
            pos.y = floor((float(int(pos.y + adjust) % uFilterGradientTileParams.y) * frac + frac / 2) * s.y);
        }
        if (uFilterGradientDirection != 0) /* horizontal */
        {
            float adjust = 0;
            if (uFilterGradientTileParams.z == 1)
                adjust = float(int(boundingBox.x) % uFilterGradientTileParams.x);
            float frac = 1 / float(uFilterGradientTileParams.x);
            pos.x = floor((float(int(pos.x + adjust) % uFilterGradientTileParams.x) * frac + frac / 2) * s.x);
        }
    }
    pos.x = max(min(pos.x, s.x - 1.0), 0.0);
    pos.y = max(min(pos.y, s.y - 1.0), 0.0);
    float gradientPos;
    if (uFilterGradientDirection == 0) /* vertical */
    {
        gradientPos = pos.y / s.y;
        if (!uFilterGradientGuiCoordinates)
            gradientPos = 1.0 - gradientPos;
    }
    else if (uFilterGradientDirection == 1) /* horizontal */
        gradientPos = pos.x / s.x;
    else if (uFilterGradientDirection == 2) /* diagonal */
    {
        vec2 center = s / 2.0;
        float angle;
        switch (uFilterGradientStartFrom)
        {
        case 0:
            angle = atan(center.y, -center.x);
            break;
    ...
    else if (uFilterGradientDirection == 3) /* rectangular */
    {
        float vert = pos.y / s.y;
        if (vert > 0.5)
            vert = 1.0 - vert;
    ...
    else /* radial */
    {
        vec2 ab = s / 2.0;
        pos -= ab;
        vec2 center = ab * uFilterGradientCenter;
        float d = distance(center, pos);
        vec2 c1 = boundingBox.xy - boundingBox.xy - ab;
        vec2 c2 = boundingBox.xw - boundingBox.xy - ab;
        vec2 c3 = boundingBox.zw - boundingBox.xy - ab;
        ...
        vec2 fs = vec2(max(abs(-ab.x + center.x), abs(ab.x + center.x)), max(abs(-ab.y + center.y), abs(ab.y + center.y)));
        float r;
        if (uFilterGradientShape == 0) // Ellipse
        {
            switch (uFilterGradientSize)
            {
            default:
            case 0: // ClosestSide
                r = filter_ellipse_radius(cs, center, pos);
                break;
            case 1: // FarthestSide
                r = filter_ellipse_radius(fs, center, pos);
                break;
            case 2: // ClosestCorner
                r = filter_ellipse_radius(abs(cc - center), center, pos);
                break;
            case 3: // FarthestCorner
                r = filter_ellipse_radius(abs(fc - center), center, pos);
                break;
            }
        }
        else if (uFilterGradientShape == 1) // Circle
        {
            switch (uFilterGradientSize)
            {
            default:
...
vec4 filter_color_at(vec2 viewPos, vec4 boundingBox)
{
    return filter_gradient_color(filter_gradient_pos_at(viewPos, boundingBox));
}
...
void standard_gradient_shader(inout vec4 color, inout vec4 function0, inout vec4 function1, inout vec4 function2, inout vec4 function3, inout vec4 function4, inout vec4 function5, inout vec4 function6)
{
    ...
    if (uFilterGradientEnabled)
    {
        /* a gradient used as a filter: it has geometry of its own, so it is evaluated here rather
           than merged into the gradient above, and it contributes alpha only, leaving the color of
           whatever is being drawn, gradient or not, as it is */
        float gf = filter_gradient_pos_at(Coord.xy, uFilterGradientBoundingBox);
        color.a *= filter_gradient_color(gf).a;
    }
}