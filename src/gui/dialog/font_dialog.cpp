...
namespace neogfx
{
    ...
    font_dialog::font_dialog(const neogfx::font& aCurrentFont, optional<text_attributes> const& aCurrentAttributes) :
        ...
        iAttributesContainer{ iLayoutEffects },
        iLayoutAttributes{ iAttributesContainer, neogfx::alignment::Top },
        iInkBox{ iLayoutAttributes, "Ink"_t },
    ...
    font_dialog::font_dialog(i_widget& aParent, const neogfx::font& aCurrentFont, optional<text_attributes> const& aCurrentAttributes) :
    ...
    optional<text_attributes> const& font_dialog::current_attributes() const
    {
        return iCurrentAttributes;
    }

    optional<text_attributes> const& font_dialog::selected_attributes() const
    {
        return iSelectedAttributes;
    }
    ...
    {
        iAttributesContainer.set_padding(neogfx::padding{});
        iLayoutAttributes.set_padding(neogfx::padding{});
        iAttributesContainer.show(iSelectedAttributes != std::nullopt);
        ...
        iSink += iInkBox.check_box().Checked([&]() { update_selected_attributes(iInkBox); });
        iSink += iInkBox.check_box().Unchecked([&]() { update_selected_attributes(iInkBox); });
        iSink += iPaperBox.check_box().Checked([&]() { update_selected_attributes(iPaperBox); });
        iSink += iPaperBox.check_box().Unchecked([&]() { update_selected_attributes(iPaperBox); });
        iSink += iAdvancedEffectsBox.check_box().Checked([&]() { update_selected_attributes(iAdvancedEffectsBox); });
        iSink += iAdvancedEffectsBox.check_box().Unchecked([&]() { update_selected_attributes(iAdvancedEffectsBox); });
        iSink += iInkColor.Checked([&]() { update_selected_attributes(iInkColor); });
        iSink += iInkGradient.Checked([&]() { update_selected_attributes(iInkGradient); });
        iSink += iInkEmoji.Toggled([&]() { update_selected_attributes(iInkEmoji); });
        iSink += iPaperColor.Checked([&]() { update_selected_attributes(iPaperColor); });
        iSink += iPaperGradient.Checked([&]() { update_selected_attributes(iPaperGradient); });
        iSink += iAdvancedEffectsOutline.Checked([&]() { update_selected_attributes(iAdvancedEffectsOutline); });
        iSink += iAdvancedEffectsShadow.Checked([&]() { update_selected_attributes(iAdvancedEffectsShadow); });
        iSink += iAdvancedEffectsGlow.Checked([&]() { update_selected_attributes(iAdvancedEffectsGlow); });
        iSink += iAdvancedEffectsColor.Checked([&]() { update_selected_attributes(iAdvancedEffectsColor); });
        iSink += iAdvancedEffectsGradient.Checked([&]() { update_selected_attributes(iAdvancedEffectsGradient); });
        iSink += iAdvancedEffectsEmoji.Toggled([&]() { update_selected_attributes(iAdvancedEffectsEmoji); });

        if (iSelectedAttributes)
        {
    ...
    void font_dialog::update_selected_attributes(i_widget const& aUpdatingWidget)
    {
        if (!iSelectedAttributes || iUpdating)
            return;
        neolib::scoped_flag sf{ iUpdating };
        auto oldSelectedAttributes = iSelectedAttributes;
        if (&aUpdatingWidget == this)
        {
            if (iSelectedAttributes)
            {
                if (iSelectedAttributes->ink() != neolib::none)
                {
                    iInkBox.check_box().check();
                    if (std::holds_alternative<color>(iSelectedAttributes->ink()))
                        iInkColor.check();
                    else if (std::holds_alternative<gradient>(iSelectedAttributes->ink()))
                        iInkGradient.check();
                }
                if (iSelectedAttributes->paper() && iSelectedAttributes->paper() != std::nullopt)
                {
                    iPaperBox.check_box().check();
                    if (std::holds_alternative<color>(*iSelectedAttributes->paper()))
                        iPaperColor.check();
                    else if (std::holds_alternative<gradient>(*iSelectedAttributes->paper()))
                        iPaperGradient.check();
                }
                iInkEmoji.set_checked(!iSelectedAttributes->ignore_emoji());
                if (iSelectedAttributes->effect())
                {
                    iAdvancedEffectsBox.check_box().check();
                    switch (iSelectedAttributes->effect()->type())
                    {
                    ...
                    if (std::holds_alternative<color>(iSelectedAttributes->effect()->color()))
                        iAdvancedEffectsColor.check();
                    else if (std::holds_alternative<gradient>(iSelectedAttributes->effect()->color()))
                        iAdvancedEffectsGradient.check();
                    iAdvancedEffectsEmoji.set_checked(!iSelectedAttributes->effect()->ignore_emoji());
                }
            }
        }
        iSelectedFont.set_underline(iUnderline.is_checked());
        if (iInkBox.check_box().is_checked())
        {
            ...
            {
                if (iSelectedAttributes->ink() == neolib::none)
                    iSelectedAttributes->set_ink(iDefaultInk ? *iDefaultInk : service<i_app>().current_style().palette().color(color_role::Text));
                else if (std::holds_alternative<gradient>(iSelectedAttributes->ink()))
                    iSelectedAttributes->set_ink(std::get<gradient>(iSelectedAttributes->ink()).color_at(0.0));
            }
            else if (iInkGradient.is_checked())
            {
                if (iSelectedAttributes->ink() == neolib::none)
                    iSelectedAttributes->set_ink(gradient{ iDefaultInk ? *iDefaultInk : service<i_app>().current_style().palette().color(color_role::Text) });
                else if (std::holds_alternative<color>(iSelectedAttributes->ink()))
                    iSelectedAttributes->set_ink(gradient{ std::get<color>(iSelectedAttributes->ink()) });
            }
            iSelectedAttributes->set_ignore_emoji(iInkEmoji.is_unchecked());
        }
        else
        {
            iSelectedAttributes->set_ink(neolib::none);
            iSelectedAttributes->set_ignore_emoji(true);
            iInk = neolib::none;
        }
        if (iPaperBox.check_box().is_checked())
        {
            ...
            {
                if (!iSelectedAttributes->paper() || iSelectedAttributes->paper() == std::nullopt)
                    iSelectedAttributes->set_paper(iDefaultPaper ? *iDefaultPaper : service<i_app>().current_style().palette().color(color_role::Background));
                else if (std::holds_alternative<gradient>(*iSelectedAttributes->paper()))
                    iSelectedAttributes->set_paper(std::get<gradient>(*iSelectedAttributes->paper()).color_at(0.0));
            }
            else if (iPaperGradient.is_checked())
            {
                if (!iSelectedAttributes->paper() || iSelectedAttributes->paper() == std::nullopt)
                    iSelectedAttributes->set_paper(gradient{ iDefaultPaper ? *iDefaultPaper : service<i_app>().current_style().palette().color(color_role::Background) });
                else if (std::holds_alternative<color>(*iSelectedAttributes->paper()))
                    iSelectedAttributes->set_paper(gradient{ std::get<color>(*iSelectedAttributes->paper()) });
            }
        }
        else
        {
            iSelectedAttributes->set_paper(std::nullopt);
            iPaper = neolib::none;
        }
        if (iAdvancedEffectsBox.check_box().is_checked())
        {
            if (iSelectedAttributes->effect() == std::nullopt)
            {
                if (iAdvancedEffectsOutline.is_checked())
                    iSelectedAttributes->effect().emplace(text_effect_type::Outline, neolib::none);
                else if (iAdvancedEffectsShadow.is_checked())
                    iSelectedAttributes->effect().emplace(text_effect_type::Shadow, neolib::none);
                else if (iAdvancedEffectsGlow.is_checked())
                    iSelectedAttributes->effect().emplace(text_effect_type::Glow, neolib::none);
            }
            else
            {
                if (iAdvancedEffectsOutline.is_checked())
                    iSelectedAttributes->effect()->set_type(text_effect_type::Outline);
                else if (iAdvancedEffectsShadow.is_checked())
                    iSelectedAttributes->effect()->set_type(text_effect_type::Shadow);
                else if (iAdvancedEffectsGlow.is_checked())
                    iSelectedAttributes->effect()->set_type(text_effect_type::Glow);
            }
            if (iAdvancedEffectsColor.is_checked())
            {
                if (iSelectedAttributes->effect()->color() == neolib::none)
                    iSelectedAttributes->effect()->set_color(iDefaultInk ? iDefaultInk->shade(0x40) : service<i_app>().current_style().palette().color(color_role::Text).shade(0x40));
                else if (std::holds_alternative<gradient>(iSelectedAttributes->effect()->color()))
                    iSelectedAttributes->effect()->set_color(std::get<gradient>(iSelectedAttributes->effect()->color()).color_at(0.0));
            }
            else if (iAdvancedEffectsGradient.is_checked())
            {
                if (iSelectedAttributes->effect()->color() == neolib::none)
                    iSelectedAttributes->effect()->set_color(gradient{ iDefaultInk ? iDefaultInk->shade(0x40) : service<i_app>().current_style().palette().color(color_role::Text).shade(0x40) });
                else if (std::holds_alternative<color>(iSelectedAttributes->effect()->color()))
                    iSelectedAttributes->effect()->set_color(gradient{ std::get<color>(iSelectedAttributes->effect()->color()) });
            }
            iSelectedAttributes->effect()->set_ignore_emoji(iAdvancedEffectsEmoji.is_unchecked());
        }
        else
        {
            iSelectedAttributes->set_effect(std::nullopt);
        ...
        {
            if (iSelectedAttributes->ink() != std::get<color_widget>(iInk).color())
                iSelectedAttributes->set_ink(std::get<color_widget>(iInk).color());
        }
        else if (std::holds_alternative<gradient_widget>(iInk) && &std::get<gradient_widget>(iInk) == &aUpdatingWidget)
        {
            if (iSelectedAttributes->ink() != std::get<gradient_widget>(iInk).gradient())
                iSelectedAttributes->set_ink(std::get<gradient_widget>(iInk).gradient());
        }
        if (&iInkEmoji == &aUpdatingWidget)
            iSelectedAttributes->set_ignore_emoji(iInkEmoji.is_unchecked());
        if (std::holds_alternative<color_widget>(iPaper) && &std::get<color_widget>(iPaper) == &aUpdatingWidget)
        {
            if (iSelectedAttributes->paper() != std::get<color_widget>(iPaper).color())
                iSelectedAttributes->set_paper(std::get<color_widget>(iPaper).color());
        }
        else if (std::holds_alternative<gradient_widget>(iPaper) && &std::get<gradient_widget>(iPaper) == &aUpdatingWidget)
        {
            if (iSelectedAttributes->paper() != std::get<gradient_widget>(iPaper).gradient())
                iSelectedAttributes->set_paper(std::get<gradient_widget>(iPaper).gradient());
        }
        if (std::holds_alternative<color_widget>(iAdvancedEffectsInk) && &std::get<color_widget>(iAdvancedEffectsInk) == &aUpdatingWidget)
        {
            if (iSelectedAttributes->effect()->color() != std::get<color_widget>(iAdvancedEffectsInk).color())
                iSelectedAttributes->effect()->set_color(std::get<color_widget>(iAdvancedEffectsInk).color());
        }
        ...
        {
            if (iSelectedAttributes->effect()->color() != std::get<gradient_widget>(iAdvancedEffectsInk).gradient())
                iSelectedAttributes->effect()->set_color(std::get<gradient_widget>(iAdvancedEffectsInk).gradient());
        }
        if (iAdvancedEffectsWidth != std::nullopt && &iAdvancedEffectsWidth->slider == &aUpdatingWidget)
        {
            if (iSelectedAttributes->effect()->width() != iAdvancedEffectsWidth->slider.value())
                iSelectedAttributes->effect()->set_width(iAdvancedEffectsWidth->slider.value());
        }
        if (&iAdvancedEffectsEmoji == &aUpdatingWidget)
            iSelectedAttributes->effect()->set_ignore_emoji(iAdvancedEffectsEmoji.is_unchecked());

        iSample.set_text_attributes(iSelectedAttributes);
        
        if (iSelectedAttributes != oldSelectedAttributes)
            SelectionChanged.trigger();
            ...
            if (iSelectedAttributes->ink() != neolib::none)
            {
                ...
                if (std::holds_alternative<color>(iSelectedAttributes->ink()))
                {
                    ...
                    if (!std::holds_alternative<color_widget>(iInk))
                    {
                        ...
                        {
                            update_selected_attributes(std::get<color_widget>(iInk));
                        });
                    }
                    std::get<color_widget>(iInk).set_color(std::get<color>(iSelectedAttributes->ink()));
                }
                else if (std::holds_alternative<gradient>(iSelectedAttributes->ink()))
                {
                    iInkGradient.check();
                    if (!std::holds_alternative<gradient_widget>(iInk))
                    {
                        ...
                        {
                            update_selected_attributes(std::get<gradient_widget>(iInk));
                        });
                    }
                    std::get<gradient_widget>(iInk).set_gradient(std::get<gradient>(iSelectedAttributes->ink()));
                }
                iInkEmoji.set_checked(!iSelectedAttributes->ignore_emoji());
            }
            else
                iInkBox.check_box().uncheck();
            if (iSelectedAttributes->paper())
            {
                iPaperBox.check_box().check();
                if (std::holds_alternative<color>(*iSelectedAttributes->paper()))
                {
                    ...
                    if (!std::holds_alternative<color_widget>(iPaper))
                    {
                        ...
                        {
                            update_selected_attributes(std::get<color_widget>(iPaper));
                        });
                    }
                    std::get<color_widget>(iPaper).set_color(std::get<color>(*iSelectedAttributes->paper()));
                }
                else if (std::holds_alternative<gradient>(*iSelectedAttributes->paper()))
                {
                    iPaperGradient.check();
                    if (!std::holds_alternative<gradient_widget>(iPaper))
                    {
                        ...
                        {
                            update_selected_attributes(std::get<gradient_widget>(iPaper));
                        });
                    }
                    std::get<gradient_widget>(iPaper).set_gradient(std::get<gradient>(*iSelectedAttributes->paper()));
                }
            }
            else
            ...
            if (iSelectedAttributes->effect() != std::nullopt)
            {
                ...
                if (std::holds_alternative<color>(iSelectedAttributes->effect()->color()))
                {
                    ...
                    if (!std::holds_alternative<color_widget>(iAdvancedEffectsInk))
                    {
                        ...
                        {
                            update_selected_attributes(std::get<color_widget>(iAdvancedEffectsInk));
                        });
                    }
                    std::get<color_widget>(iAdvancedEffectsInk).set_color(std::get<color>(iSelectedAttributes->effect()->color()));
                }
                else if (std::holds_alternative<gradient>(iSelectedAttributes->effect()->color()))
                {
                    iAdvancedEffectsGradient.check();
                    if (!std::holds_alternative<gradient_widget>(iAdvancedEffectsInk))
                    {
                        ...
                        {
                            update_selected_attributes(std::get<gradient_widget>(iAdvancedEffectsInk));
                        });
                    }
                    std::get<gradient_widget>(iAdvancedEffectsInk).set_gradient(std::get<gradient>(iSelectedAttributes->effect()->color()));
                }
                if (iAdvancedEffectsWidth == std::nullopt)
                {
                    ...
                    {
                        update_selected_attributes(iAdvancedEffectsWidth->slider);
                    });
                ...
                iAdvancedEffectsWidth->slider.set_value(static_cast<int32_t>(iSelectedAttributes->effect()->width()));
                iAdvancedEffectsEmoji.set_checked(!iSelectedAttributes->effect()->ignore_emoji());
            }
            else
