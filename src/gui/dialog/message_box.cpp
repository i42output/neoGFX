...
namespace neogfx
{
    ...
    dialog_result message_box::exec()
    {
        ...
        if (!has_response())
            set_response(button_box().button_with_role(result != dialog_result::Rejected ? button_role::Accept : button_role::Reject));
        return result;
    }
    ...
    void message_box::init()
    {
        ...
        title_bar().title_widget().set_flags(title_bar().title_widget().flags() & ~(text_widget_flags::UseEllipsis | text_widget_flags::UseFade));
        ...
        text().set_font(text().font().with_style(font_style::Bold));
