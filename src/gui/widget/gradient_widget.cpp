...
namespace neogfx
{
    ...
    void gradient_widget::paint(i_graphics_context& aGc) const
    {
        ...
        if (iCurrentColorStop != std::nullopt)
        {
            ...
            aGc.draw_line(point{ std::floor(r.center().x), r.top() }, point{ std::floor(r.center().x), contents_rect().top() }, 
                pen{ color::White, line_dash{ 0x5555u, 1.0_dip } });
    ...
    void gradient_widget::mouse_button_double_clicked(mouse_button aButton, point const& aPosition, key_modifier aKeyModifier)
    {
        ...
        if (aButton == mouse_button::Left)
        {
            ...
            if (std::holds_alternative<gradient::color_stop_list::iterator>(stopIter))
            {
                ...
                auto cd = std::make_unique<color_dialog>(*this, stop.second());
    ...
    void gradient_widget::mouse_button_released(mouse_button aButton, point const& aPosition)
    {
        ...
        {
            auto moreAction = make_ref<action>("More..."_t);
            moreAction->Triggered([this]()
            {
                neogfx::gradient const originalGradient = gradient();
                auto gd = std::make_unique<gradient_dialog>(*this, gradient());
                if (iCustomColors != std::nullopt)
            ...
            if (!std::holds_alternative<std::monostate>(stopIter) && stopIter == stop_at(*iClicked))
            {
                if (std::holds_alternative<gradient::color_stop_list::iterator>(stopIter))
                {
                    ...
                    auto selectColorAction = make_ref<action>("Select stop color..."_t);
                ...
                else if (std::holds_alternative<gradient::alpha_stop_list::iterator>(stopIter))
                {
                    ...
                    auto selectAlphaAction = make_ref<action>("Select stop alpha (opacity level)..."_t);
