...
namespace neogfx
{
    ...
    // the position of channel within an interleaved frame of channels; channel must be a single channel present in channels
    inline std::uint64_t channel_index(audio_channel channels, audio_channel channel)
    {
        return static_cast<std::uint64_t>(std::popcount(static_cast<std::uint64_t>(channels) & (static_cast<std::uint64_t>(channel) - 1ULL)));
    }
