...
namespace neogfx
{
    ...
    struct rendering_context_slow_state : rendering_context_state
    {
        ...
        // glCullFace depends on the coordinate system's handedness as well as on the
        // requested mode, so the resolved flip has to be part of the cache key
        std::optional<bool> faceCullingFlipped;
